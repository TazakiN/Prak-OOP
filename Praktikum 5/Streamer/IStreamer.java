...
public interface IStreamer<T> {
    ...
    // mengembalikan stream baru yang berisi hasil kembalian fungsi mapper untuk
    // setiap elemen dalam stream
    <R> IStreamer<R> map(Function<T, R> mapper);

    // melakukan filter terhadap elemen dalam stream, jika fungsi predicate
    // mengembalikan true, elemen tersebut akan dimasukkan ke dalam stream baru,
    // jika false, elemen tersebut akan diabaikan
    ...
    // reducer menerima hasil reduksi sampai saat ini dan mengembalikan fungsi yang
    // menerima elemen
    // stream dan mengembalikan hasil reduksi setelah elemen stream tersebut di
    // proses Contoh:
    // stream.reduce(0.0, (Double acc) -> (Double item) -> acc + item);
    <R> R reduce(R initialValue, Function<R, Function<T, R>> reducer);

    // panggil fungsi mapper untuk setiap elemen dalam stream, kemudian gabungkan
    // hasilnya menjadi satu stream baru
    // misalkan stream berisi [1, 2, 3] dan mapper mengembalikan stream baru [1, 2,
    // 3] untuk setiap elemen, maka hasilnya adalah stream baru [1, 2, 3, 1, 2, 3,
    // 1, 2, 3]
    <R> IStreamer<R> flatMap(Function<T, IStreamer<R>> mapper);

    // mengembalikan elemen pertama dalam stream yang memenuhi kondisi dari fungsi
    // predicate
    // jika tidak ada elemen yang memenuhi kondisi, kembalikan null
    T findFirst(Function<T, Boolean> predicate);

    // mengembalikan elemen terakhir dalam stream yang memenuhi kondisi dari fungsi
    // predicate
    // jika tidak ada elemen yang memenuhi kondisi, kembalikan null
    ...
    // mengembalikan stream baru yang berisi semua elemen dalam stream kecuali
    // elemen pertama
    ...
    // jika n lebih besar dari jumlah elemen dalam stream, ambil semua elemen dalam
    // stream
    ...
    // mengembalikan true jika stream mengandung minimal satu elemen yang memenuhi
    // kondisi dari fungsi predicate, false jika tidak
    Boolean some(Function<T, Boolean> predicate);

    // mengembalikan true jika semua elemen dalam stream memenuhi kondisi dari
    // fungsi predicate, false jika tidak
