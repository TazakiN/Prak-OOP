...
class ArrayManipulation {
     ...
     * Anda dapat menggunakan {@link Arrays#sort(Object [])} untuk mengurutkan
     * array, tidak perlu
    ...
    public static <T extends Comparable<T>> T[] mergeAndSortArrays(T[] arr1, T[] arr2) {
        T[] mergedArr = Arrays.copyOf(arr1, arr1.length + arr2.length);
     ...
     * Anda dapat menggunakan {@link Comparable#compareTo(Object)} untuk
    ...
    /**
     * Menerapkan fungsi mapper ke setiap elemen dalam array dan mengembalikan array
     * hasilnya.
     * Anda dapat menggunakan {@link Function#apply(Object)} untuk menerapkan fungsi
     * mapper ke
     ...
     * @return array hasil dari menerapkan fungsi mapper ke setiap elemen dalam
     *         array masukan
     */
    public static <T> T[] mapArray(T[] arr, Function<T, T> mapper) {
        T[] arrHasil = Arrays.copyOf(arr, arr.length);
        for (int i = 0; i < arr.length; i++) {
            arrHasil[i] = mapper.apply(arr[i]);
        }
        return arrHasil;
    }

    /**
     * Mengembalikan array baru yang berisi elemen dari array masukan yang memenuhi
     * kondisi yang
     * ditentukan oleh predicate. Anda dapat menggunakan
     * {@link IntPredicate#test(int)} untuk
     * menentukan apakah elemen ke-i memenuhi kondisi yang ditentukan oleh
     * predicate. Contoh:
     * predicate.test(i).
     * Array masukan tidak mungkin null.
     * Pastikan isi dari array input tidak berubah.
     ...
     * @return array baru yang berisi elemen dari array masukan yang memenuhi
     *         kondisi yang
     */
    ...
    public static <T> T[] filterArray(T[] arr, Predicate<T> predicate) {
        ...
        T[] arrHasil = Arrays.copyOf(arr, count);
        int index = 0;
        for (T t : arr) {
            if (predicate.test(t)) {
                arrHasil[index] = t;
